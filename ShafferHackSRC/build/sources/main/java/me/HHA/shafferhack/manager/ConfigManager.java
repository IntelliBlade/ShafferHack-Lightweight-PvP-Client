...
import me.HHA.shafferhack.features.setting.Bind;
import me.HHA.shafferhack.features.setting.EnumConverter;
import me.HHA.shafferhack.features.setting.Setting;
import me.HHA.shafferhack.util.Util;
...
public class ConfigManager implements Util {
    ...
    public String config = "shafferhack/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        shafferhack.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    shafferhack.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                continue;
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("shafferhack").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("shafferhack/" + name + "/"))) {
            this.config = "shafferhack/" + name + "/";
        } else {
            this.config = "shafferhack/config/";
        }
        shafferhack.friendManager.onLoad();
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("shafferhack").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("shafferhack/" + name + "/"));
    }
    ...
    public void saveConfig(String name) {
        ...
        if (!path.exists())
            path.mkdir();
        shafferhack.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("shafferhack/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("shafferhack", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(shafferhack.moduleManager.modules);
        this.features.add(shafferhack.friendManager);
        String name = loadCurrentConfig();
        loadConfig(name);
        shafferhack.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            shafferhack.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
