...
public class AutoWeb
    ...
    public AutoWeb() {
        super("AutoWeb", "Traps other players in webs", Category.OTHER, true, false, false);
    }
    ...
    private EntityPlayer getTarget(double range) {
        ...
        for (EntityPlayer player : AutoWeb.mc.world.playerEntities) {
            if (EntityUtil.isntValid(player, range) || player.isInWeb || shafferhack.speedManager.getPlayerSpeed(player) > 30.0)
                continue;
            if (target == null) {
                target = player;
