package me.HHA.shafferhack.features.modules.client;

import com.mojang.realmsclient.gui.ChatFormatting;
import me.HHA.shafferhack.shafferhack;
import me.HHA.shafferhack.event.events.ClientEvent;
...
import me.HHA.shafferhack.util.*;
...
public class HUD extends Module {
    ...
    public Setting<String> command = register(new Setting("Command", "ShafferHack"));
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (this.arrayList.getValue().booleanValue())
            if (this.renderingUp.getValue().booleanValue()) {
                if (this.renderingMode.getValue() == RenderingMode.ABC) {
                    for (int k = 0; k < shafferhack.moduleManager.sortedModulesABC.size(); k++) {
                        String str = shafferhack.moduleManager.sortedModulesABC.get(k);
                    ...
                    for (int k = 0; k < shafferhack.moduleManager.sortedModules.size(); k++) {
                        Module module = shafferhack.moduleManager.sortedModules.get(k);
        ...
        if (this.renderingUp.getValue().booleanValue()) {
            if (this.potions.getValue().booleanValue()) {
                ...
                for (PotionEffect potionEffect : effects) {
                    String str = shafferhack.potionManager.getColoredPotionString(potionEffect);
                    i += 10;
            ...
            if (this.speed.getValue().booleanValue()) {
                String str = grayString + "Speed " + ChatFormatting.WHITE + shafferhack.speedManager.getSpeedKpH() + " km/h";
                i += 10;
            ...
            if (this.tps.getValue().booleanValue()) {
                String str = grayString + "TPS " + ChatFormatting.WHITE + shafferhack.serverManager.getTPS();
                i += 10;
                this.renderer.drawString(str, (width - this.renderer.getStringWidth(str) - 2), (height - 2 - i), (ClickGui.getInstance()).rainbow.getValue().booleanValue() ? (((ClickGui.getInstance()).rainbowModeA.getValue() == ClickGui.rainbowModeArray.Up) ? ColorUtil.rainbow(counter1[0] * (ClickGui.getInstance()).rainbowHue.getValue().intValue()).getRGB() : ColorUtil.rainbow((ClickGui.getInstance()).rainbowHue.getValue().intValue()).getRGB()) : this.color, true);
                counter1[0] = counter1[0] + 1;
            }
            String fpsText = grayString + "FPS " + ChatFormatting.WHITE + Minecraft.debugFPS;
            String str1 = grayString + "Ping " + ChatFormatting.WHITE + shafferhack.serverManager.getPing();
        ...
        String direction = this.direction.getValue().booleanValue() ? shafferhack.rotationManager.getDirection4D(false) : "";
    ...
    public void renderLag() {
        int width = this.renderer.scaledWidth;
        if (shafferhack.serverManager.isServerNotResponding()) {
            String text = ChatFormatting.RED + "Server not responding " + MathUtil.round((float) shafferhack.serverManager.serverRespondingTime() / 1000.0F, 1) + "s.";
    ...
    public void onLoad() {
        shafferhack.commandManager.setClientMessage(getCommandMessage());
    }
